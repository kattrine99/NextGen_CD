...
def goForward(duration, speed = 0.5):
	print("Moving forward for "+ str(duration)+" s "
...
def goBackward(duration, speed = 0.5):
	print("Moving backward for "+ str(duration)+" s "
		+"with speed = "+str(speed))
	left.backward(speed)
...
def goSmoothLeft(duration, leftCoef = 0.1 , rightCoef = 0.5):
	print("Moving smooth left for "+ str(duration)+" s, "
		+"leftCoef = "+ str(leftCoef) + ", rightCoef = " + str(rightCoef))
...
def goSmoothRight(duration, leftCoef = 0.5 , rightCoef = 0.1):
	print("Moving smooth right for "+ str(duration)+" s, "
		+"leftCoef = "+ str(leftCoef) + ", rightCoef = " + str(rightCoef))
	left.forward(leftCoef)
  	right.forward(rightCoef)
  	time.sleep(duration)
...
def goSmoothBackLeft(duration, leftCoef = 0.1 , rightCoef = 0.5):
	print("Moving smooth back left for "+ str(duration)+" s, "
		+"leftCoef = "+ str(leftCoef) + ", rightCoef = " + str(rightCoef))
...
def goSmoothBackRight(duration, leftCoef = 0.5 , rightCoef = 0.1):
	print("Moving smooth back right for "+ str(duration)+" s, "
		+"leftCoef = "+ str(leftCoef) + ", rightCoef = " + str(rightCoef))
	left.backward(leftCoef)
  	right.backward(rightCoef)
  	time.sleep(duration)
